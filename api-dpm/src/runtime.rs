...
impl DpmSvr {
    ...
    /// 新增一个runtime类型
    /// HTTP POST
    pub async fn post_addruntime(
        &self,
        runtime_name: &str,
        ver_str: &str,
    ) -> Result<(bool, RespVO<serde_json::Value>), error::ApiReqError> {
        let addr = url::Url::parse(&self.host)?.join("/api/dpm/runtime/add")?;
        //cli
        ...
        let resp = cli
            ...
            .await?;
        let r = middler::map_respvo_iserror_vo(resp)?;
        Ok(r)
    }
    ...
    pub async fn get_runtime_list(
        ...
        let mut addr = url::Url::parse(&self.host)?.join("/api/dpm/runtime/list")?;
    ...
    pub async fn get_runtime_one(
        ...
        //cli
        let cli = self.client_builder()?;
        let path = format!("/api/dpm/runtime/one?name={name}", name = name,);
