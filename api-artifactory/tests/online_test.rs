...
async fn start() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_ne!(resp_vo.code == 0, resp_vo.code == 1001, "--->> add runtime failed!");
    ...
    assert_ne!(resp.code == 0, resp.code == 1001, "--->> add artifactory failed!");
    ...
    let pkg : ArtifactoryCellInfo = serde_json::from_value(resp.data.expect("ENSURE")).expect("parse failed！");
    let resp = svr.get_artifactory_download_url(pkg.pid).await?;
    println!("[done!]get presigned url: {:#?} done!", resp);    
    
    // try download
    let download_url: PresignedUrl = serde_json::from_value(resp.data.expect("ENSURE")).expect("parse failed！");
